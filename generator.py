...
def make_generator_model():
    ...
    #Upscale using Upsampling2D and Convolution2D
    model.add(layers.UpSampling2D(size=(2, 2), interpolation='nearest'))
    model.add(layers.Conv2D(512, (5, 5), padding='same', use_bias=False,
    ...
    model.add(layers.UpSampling2D(size=(2, 2), interpolation='nearest'))
    model.add(layers.Conv2D(256, (5, 5), padding='same', use_bias=False,
                            kernel_initializer=weight_initializer))
    ...
    model.add(layers.Conv2D(3, (5, 5), padding='same', use_bias=False,
                            kernel_initializer=weight_initializer, activation='tanh'))
